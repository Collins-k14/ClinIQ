...
router.post('/', requireAuth, getUserInfo, async (req, res) => {
  try {
    ...
    const appointment = new Appointment({
      clerkId: req.userId, // automatically set from Clerk token
      facilityId,
      service,
      date,
      time
...
router.get('/', requireAuth, async (req, res) => {
  try {
    const appointments = await Appointment.find({ clerkId: req.userId })
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const appointment = await Appointment.findById(req.params.id)
      .populate('facilityId', 'name address phone services rating reviews');

    if (!appointment) {
      return res.status(404).json({ error: 'Appointment not found' });
    }

    // Ensure the user can only access their own appointment
    if (appointment.ClerkId !== req.userId) {
      return res.status(403).json({ error: 'Forbidden' });
    }
...
router.put('/:id/cancel', requireAuth, async (req, res) => {
  try {
    ...
    if (appointment.clerkIdId !== req.userId) {
      return res.status(403).json({ error: 'Forbidden' });
    }
